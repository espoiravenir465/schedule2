 ...
 <div class="event-container">
  ...
  <table class="table bordered break-all">
	...
	<tbody>
        ...
        <input v-if="event.start_edit" type="time" step="900" value="00:00" v-model="event.start" v-on:blur="$set(event, 'start_edit', false)"  >
			</td>
			<td align = "center" valign ="middle">
			  <div v-if="!event.end" v-text="event.end" v-on:click="$set(event, 'end_edit', true)"></div>
        <input v-if="event.end_edit" type="time" step="900" value="00:00" v-model="event.end" v-on:blur="$set(event, 'end_edit', false)"  >
			</td>
			<td align = "center" valign ="middle">
